# -*- coding: utf-8 -*-
"""
Build dense-only (FAISS) RAG index for sleep textbooks with minimal-section splitting:
...
- Minimal sections: within each Chapter (L1) span, boundaries are L1 itself and any L2/L3 headings
  that are listed in the TOC for that Chapter and actually appear in the Markdown.
- Each minimal section is then split by a char-window (fast path), keeping small overlaps.
- Outputs: chunks.parquet, faiss.index, meta.json
- Search: search_dense() returns top chunks by cosine similarity from FAISS.
"""
...
OUT_DIR = BOOK_DIR / "rag_index_1105_dense_only"
...
def build_minimal_sections(book_name: str) -> List[Dict]:
        ...
        for raw1, md_raw1, n1, s, e in l1_spans:
            ...
            for i in range(len(local_boundaries)):
                ...
                sections.append({
                    ...
                    "breadcrumb_str": " ".join(local_boundaries[i]["breadcrumb"]) if isinstance(local_boundaries[i]["breadcrumb"], list) else str(local_boundaries[i]["breadcrumb"]),
                    "start_char": int(bs),
                    "end_char": int(be),
                })
        return sections
    ...
    if not assigned:
        ...
        assigned = [{"raw": h["raw"], "breadcrumb": [h["raw"]], "start_char": h["start_char"]} for h in all_md_heads]
...
def char_based_chunk(
    ...
    """
    Character-window splitter (fast path).
    Returns (chunk_text, start_pos, end_pos, pseudo_token_len)
    """
    text = text or ""
    n = len(text)
    if n == 0:
...
def build_faiss_index(embs: np.ndarray):
    d = embs.shape[1]
    index = faiss.IndexFlatIP(d)  # normalized -> cosine similarity via inner product
    index.add(embs)
    return index
...
def main():
    ...
    log("[Done] Dense-only index is ready.")


# -------------------- Dense-only search helper --------------------
def search_dense(query: str, topn: int = 12):
    """
    Dense-only retrieval via FAISS cosine similarity.
    Returns topn chunks with scores and brief preview.
    """
    chunks_path = OUT_DIR / "chunks.parquet"
    ...
    D, I = faiss_index.search(np.asarray(q_emb, dtype="float32"), topn)
    out = []
...
if __name__ == "__main__":
    main()
    # quick test:
    print(search_dense("diagnostic criteria for insomnia disorder", topn=8))
